...
| 非结构化文档信息提取 | M2 | AI组 | `src/core/extraction_service.py`、`parser_factory` / `docling_adapter`、`langextract_adapter` |
| 表格自定义数据填写 | M3 | AI组 | `src/core/writers.py`、`src/core/output_writer_orchestrator.py`、`src/core/alias.py` |
| 数据存储与管理 | DB | 数据库组 | 调用 AI 组输出接口 |
| 用户界面与交互 | WEB | 网页组 | 调用 `api_server.py` HTTP API（跨域与 `A23_CORS_*` 见根目录 `HTTP_API_USAGE.md`） |
...
**接入点**（以 `HTTP_API_USAGE.md` 为准）：
- **主路径**：`POST /api/extract/direct`、`POST /api/extract/no-template`、`POST /api/extract/pre-analyze` 等
- **可选**：仅当部署开启 `A23_ENABLE_TASKS=true` 时才有 `POST /api/tasks/*` 与 `GET /api/tasks/{id}/stream`；生产默认关闭，异步编排由业务后端承担
...
- 字段名归一依赖 `src/core/alias.py` 与 `src/knowledge/field_aliases.json`（及配置化规则）
- 输出 records 中每条记录的字段名必须与模板字段名严格一致
- 模型 HTTP：默认首包 120s、重试每次 +60s（最多 3 次）；代码路径可通过 `call_model(..., timeout=秒)` 收紧单次请求（见 `src/adapters/model_client.py`）
