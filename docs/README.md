...
| [../HTTP_API_USAGE.md](../HTTP_API_USAGE.md) | **生产环境**：FastAPI 接口、**环境变量唯一权威表**（含 CORS、`A23_QNA_*`、文档问答 `POST /api/qna/ask` 等） |
...
命令行 `main.py` 用于调试；`scripts/` 下其余脚本多为本地批测（仓库默认不纳入 Git），与 API 共用的运维脚本见仓库内已跟踪的 `scripts/*.py` 说明。生产主路径为 **HTTP API**；可选异步任务见 `src/api/task_manager.py`（子进程调用 `main.py`）。
