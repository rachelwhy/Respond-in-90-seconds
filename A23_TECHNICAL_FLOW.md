...
├── src/core/reader.py（collect_input_bundle、表格直读 try_internal_structured_extract）
├── src/core/writers.py（Excel/Word 等写回）
└── src/core/coverage_guard.py（可选辅助模块；主流程是否调用以 main.py 为准）
...
系统通过`src/core/reader.py`的`collect_input_bundle()`函数统一处理输入：
- **Excel**：使用openpyxl解析，保留表格结构
- **Word**：使用python-docx，支持多表格识别
- **Markdown/纯文本**：直接读取，保留原始格式
- **PDF**：通过pdf2image+Pillow+Tesseract OCR（可选）
- **图像**：Tesseract OCR（需单独安装系统依赖）
...
| **supplement** | 规则预抽取 + AI补充缺失字段 | 大部分字段可规则抽取，少量需要AI补全 |
...
config = {
    "llm_mode": llm_mode,  # 'full' / 'supplement'
...
| **表格数据补全** | Word模板 | 半结构化 | supplement | ✅ 可能使用 | 规则预抽取 → AI补充缺失字段 |
...
- **表格数据补全**：`--llm-mode supplement --template template.xlsx`
