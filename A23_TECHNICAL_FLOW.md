...
### 1.1 核心功能定位
A23 AI 文档处理系统是企业内网文档结构化处理工具，核心功能为**模板填表**，可选功能为**文档问答**。系统采用混合策略：规则预抽取提供确定性，AI 模型验证提供语义理解，并由**显式路由与配置**（如 `extraction_routing`、`llm_mode`、环境变量）约束行为边界与可用性。
...
└── src/api/
    ...
    ├── qna_service.py（文档问答编排）
    ├── qna_langchain.py（LangChain + Chroma + HF 嵌入，默认）
    └── qna_retrieval.py（BM25 + 句向量混合检索，回退）

核心业务与抽取
├── src/core/extraction_service.py（切片与智能抽取主逻辑）
├── src/core/model_extraction_orchestrator.py（CLI/API 共享模型编排主入口）
├── src/core/extraction_result_harmonizer.py（模型输出与内部结构化结果合流）
├── src/core/result_packager.py（CLI/API 共享结果打包与元信息组装）
├── src/core/output_writer_orchestrator.py（CLI/API 共享模板写回与输出文件组装）
├── src/core/runtime_observability.py（CLI/API 共享 runtime 指标收口与摘要）
...
├── src/observability/（Prometheus 等指标，可选）
...
- 入口：`main.py` 与 `src/api/direct_extractor.py`
- 模型阶段：`src/core/model_extraction_orchestrator.py::run_model_extraction_path`
- 结果合流：`src/core/extraction_result_harmonizer.py`
- 结果打包：`src/core/result_packager.py`
- 输出写回：`src/core/output_writer_orchestrator.py`
- 运行指标：`src/core/runtime_observability.py`

**`extractor.py` 核心逻辑（能力底座）**：
```python
...
> 说明：LangExtract 的调用与是否启用由统一编排链路决定，历史上分散在多个入口的阶段函数已收敛到共享 orchestrator/harmonizer。
