...
| P1 | 仓库卫生 | `tests/`、运行期缓存/向量库、`models/qna_embedding/` 等默认 **不纳入 Git**；`scripts/` 仅跟踪运维脚本（如句向量下载与依赖自检），其余批测脚本本地保留 |
...
1. **文档对齐**：`README.md` / `A23_TECHNICAL_FLOW.md` / `HTTP_API_USAGE.md` 已与当前目录及超时相关环境变量同步（见持续提交）；若再变动模块树请同步三处。
2. **`.gitignore`**：已补充常见本地噪音与 `*_result*.xlsx` 等调试表格模式；可按团队习惯收紧/放宽。
3. **测试修复 PR**：单独分支，`pytest tests/unit/` 仍有历史失败用例，需逐模块修复或标记 xfail（仍未作为 CI 门禁）。
...
| 2026-04-21 | 跟进：目录树补 `observability`、HTTP 文档补超时类变量、`.gitignore` 补强 |
| 2026-05-07 | 文档收敛：`HTTP_API_USAGE.md` 为环境变量单一事实来源；补充 CORS、问答 `A23_QNA_*`；`.gitignore` 调整 `scripts/` 跟踪策略 |
